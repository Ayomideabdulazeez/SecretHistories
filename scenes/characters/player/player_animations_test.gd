...
@export_enum("IDLE", "ADS", "RELOAD") var weapon_state = "IDLE":
	set(value):
...
@onready var main_hand_equipment_root = get_node("%MainHandEquipmentRoot")


func change_gun(value : String) -> void:
	...
	if get_equipped_weapon():
		get_equipped_weapon().queue_free()
	main_hand_equipment_root.add_child(spawned_weapon)
	weapon_state = "IDLE"
...
func adjust_weapon_position(value) -> void:
	if not Engine.is_editor_hint():
		return
...
func reset_animation_tree(value) -> void:
	if not Engine.is_editor_hint():
		return
...
func get_equipped_weapon() -> EquipmentItem:
	var equipped_weapon : Object
	for available_weapon in main_hand_equipment_root.get_children():
		if available_weapon is EquipmentItem:
			equipped_weapon = available_weapon
	return equipped_weapon

		#animation_tree_tweener("parameters/IdleWalkRun/blend_amount", 0.0, 0.1)

func operation_tween(object, method, tweening_from, tweening_to, duration) -> void:
	var tweener = get_tree().create_tween()
	tweener.tween_property(animation_tree, method, tweening_to, duration)

##Used for interpolation between different animations in the animation tree
func animation_tree_tweener(animation_property, final_animation_value : float, duration: float) -> void:
	var tweener = get_tree().create_tween()
	tweener.tween_property(animation_tree, animation_property, final_animation_value, duration)

# TODO: haven't tested since changing tween
func adjust_arm(final_position : Vector3) -> void:
